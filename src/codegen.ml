...
let const_decls : (L.llvalue * sconst_decl) StringMap.t = 
  ...
  let build_constructor_body cdecl =
      let ct = cdecl.sconst_decl in
      let class_name = cdecl.cname in 
      let (the_constructor, _) = StringMap.find (class_name ^ "_constructor") const_decls in
	...
	in StringMap.add n local_var m
      in
      let formals = List.fold_left2 add_formal StringMap.empty ct.sctformals
          (Array.to_list (L.params the_constructor)) in
      List.fold_left add_local formals ct.sctlocals
    in
  ...
  List.iter build_constructor_body classes;
  the_module
